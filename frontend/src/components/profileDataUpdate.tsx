...
export const UpdateUserData = ({ open, onClose, updateType }: UpdateUserDataProps) => {
    ...
    const handleNameSubmit = async () => {
        ...
        const response = await fetch('https://localhost:8443/user/updateName', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ userID: user.userID, name: formData.name })
        });
        if (response.ok) {
            ...
            setErrors((prev : UpdateUserDataProps) => ({ ...prev, general: errorData.message || 'Failed to update nickname' }));
            setLoading(false);
        }
    };
    ...
    const handleEmailSubmit = async () => {
        ...
        const response = await fetch('https://localhost:8443/user/updateEmail', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ userID: user.userID, email: formData.email })
        });
        ...
        if (response.ok) {
            ...
            const errorData = await response.json();
            setErrors((prev : UpdateUserDataProps) => ({ ...prev, general: errorData.message || 'Failed to update email' }));
            setLoading(false);
        }
    };
