...
export function PlayerSelection({ open, users, currentUserName, onSelect, onCancel }: PlayerSelectionProps) {
    if (!open) return null;
    return (
                    ...
                    {users
                        ...
                        .map((user) => (
                            ...
                            >
                                {user.avatarUrl && (
                                    <img 
                                        src={user.avatarUrl.startsWith('/') ? `https://localhost:3000${user.avatarUrl}` : user.avatarUrl}
                                        alt={user.name} 
